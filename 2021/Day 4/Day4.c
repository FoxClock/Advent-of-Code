...
struct board
{
    ...
    int daubedNumbers[GRIDSIZE][GRIDSIZE];
    int sum;
}board;
...
{
    // Variables
    size_t lenOfMoves, numOfBoards;         // Number of moves & the number of boards
    int moves[BUFFER];                      // Int array for moves
    struct board boards[BUFFER/2];          // array of board structs

    // get user input for file name
    printf("Please enter file name: \n");
    char *filename = "input.txt";       // BUG: Cannot use user input for filename
