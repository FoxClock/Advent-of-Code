...
func subtractSmallestPairs(valueList: valueLists) -> [Int] {
    ...
    for _ in 0..<valueList.left_list.count {
        // Get the minimum value on the left list
        leftSmallest = leftList.min()!

        // Count the number of values which match the left list small value
        countOfValues = rightList.count { Int in
            Int == leftSmallest
        }
        ...
        let difference = abs(leftSmallest * countOfValues)
